param(
  ...
  [string]$StartDate = '2026-01-01',  # YTD from start of year
...
Write-Output "Refreshing MIDAS submissions data (YTD only, starting $StartDate)..."
...
Write-Output "Connecting to $Server\\$Database..."
...
Write-Output "Retrieved $($dt.Rows.Count) rows from database"
...
if ($futureRows.Count -gt 0) {
  Write-Warning ("Excluded {0} MIDAS row(s) with DateCreated after {1}" -f $futureRows.Count, $maxAllowedDate.ToString('yyyy-MM-dd'))
}
...
Write-Output "[SUCCESS] Updated midas_submissions_data.js -> rows: $($rows.Count), excludedFutureRows: $($futureRows.Count), startDate: $StartDate, fileSizeMB: $fileSizeMB"
